...
const Sidebar = ({ data, newData, id }: any) => {
	...
	const filteredData: userData[] = fetchedData?.filter((item: any) => item?.id === id);
	...
	const handleSubmit = async (event: React.FormEvent) => {
		...
		try {
			...
			image.onload = () => {
				...
				if (width !== height) {
					toast.error("Please upload an image with equal width and height (square aspect ratio).");
					return;
				}
						...
						{key.user_id == id ? (
							<div>
								<div className='relative'>
									...
									<label htmlFor="image-upload" className="absolute bottom-0 left-1/2 transform -translate-x-1/2 p-1 bg-gray-800 rounded-full cursor-pointer">
								...
								{showButtons && (
									<div className={`${cancel}  items-center space-x-4 mt-4 justify-center w-full`}>
										<button
											className={"text-yellow-500 hover:text-green-600 font-bold text-xl"}
											disabled={loading}
										...
										<button className="text-red-500 hover:text-red-700 font-bold text-xl" onClick={handleCancel}><X /></button>
									</div>
								)}



								<h1 className='text-4xl sm:text-6xl mb-2'>{key.fname} {key.lname}</h1>
								<h3 className='mb-8'>{filteredData[0]?.role.toUpperCase()}</h3>
								{filteredData && filteredData[0]?.role === 'hr' ? (
									<Rating id={id} />
								) : filteredData && filteredData[0]?.role === 'user' && (
									<RatingUser id={id} />
								)}
								{userData && userData?.role != 'hr' && userData?.id != id && (
									<button className={`px-5 py-2 ${userData.id == id ? "hidden" : ""} ${userData.role == 'hr' ? "hidden" : ""} ${key.role == 'user' && userData.role == 'user' ? "hidden" : ""} bg-[#68F590] hover:bg-[#FAFAFA] rounded-lg text-black font-bold my-5`} disabled={loading} onClick={onClickHandler} type="submit">{loading ? "Loading" : "Schedule Now"}</button>
								)}
								<h2>Education</h2>
								{filteredQualificaton.map((item: any) => (
									<div key={item.id}>
										...
										<p>{item.cgpa} ({item.passing_year})</p>
									</div>
								))
								}
