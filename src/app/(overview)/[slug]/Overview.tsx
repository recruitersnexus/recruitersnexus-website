...
const Overview = ({slug,id,filteredHr,filterServices,allInterviews}:any) => {
  ...
  return (
    <div className={`w-full flex md:flex-row flex-col pt-12 bg-[#F2F5F9] font-nunito`}>
        ...
        <div className={`w-full ${useUserData().userData? "md:flex": "hidden"}  md:min-w-[250px] md:w-1/5 hidden  flex-col p-6 h-full  md:h-auto bg-[#F2F5F9] border-r-2 border-[#A4AC7EB] space-y-6`}>
          <SideBar slug={slug} />
        </div>

        <div className={`w-full ${useUserData().userData? "md:w-4/5":"w-full"} h-full bg-[#F2F5F9] pb-32`}>
          {slug === "dashboards" ? (
            <Dashboard />
          ) : slug === "exploreExpert" ? (
            <Explore />
            ...
            <ScheduleInterview hr_id={id} filteredHr={filteredHr} filterServices={filterServices} allInterviews={allInterviews} />
          ) :  slug === "posting" ? (
            <Posting />
          ):  slug === "all-accounts" ? (
            <AllAccounts />
        ...
        <div className={`w-full ${status === "404"? "hidden": "flex"} fixed bottom-0 z-20 md:min-w-[250px] border-t-2  md:w-1/5  md:hidden items-center justify-center flex-col py-0 h-24  md:h-auto bg-[#F2F5F9] border-r-2 border-[#A4AC7EB] space-y-6`}>
          <SideBar slug={slug} />
        </div>
