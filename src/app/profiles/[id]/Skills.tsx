...
const Modal = ({ onClose }: { onClose: () => void }) => {
	...
	const filterSkills = skills?.filter((item:any)=>item?.user_id === userData?.id);

	async function onAddSkill() {
		try {
			if(filterSkills.some((item:any)=>item?.skill.toLowerCase() === skill.toLowerCase()))
		{
...
const Skills = ({ data, skillsData, newData, id }: any) => {
	...
	const filteredVerifications = verification?.filter((item:any)=>userData?.id === item?.user_id);
	const filteredUsers =  users?.filter((item:any)=>id === item?.id);
	...
	const tabs = (
	  <div className={`flex ${filteredUsers && filteredUsers[0]?.role == "hr" ? "hidden" : ""}`}>
