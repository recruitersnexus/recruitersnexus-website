...
const Rating = ({ id }: any) => {
    ...
    const filteredFeedback = feedback?.filter((item: any) => item?.hr_id === id && item?.total_rating != null);

    //console.log("Feedback Filtered On Rating Comp: ", filteredFeedback);
    
    // Calculate average total_rating
    const totalRatings = filteredFeedback.map((item: any) => parseFloat(item.total_rating));
    ...
    //console.log("Type of totalRatings: ", typeof totalRatings.at(0));
